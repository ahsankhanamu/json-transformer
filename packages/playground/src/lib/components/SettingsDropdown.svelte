...
<div class="relative flex items-center" bind:this={dropdownRef}>
  ...
  <div class="flex items-center rounded-lg overflow-hidden border border-[var(--color-border)]">
    ...
    >
      ...
      <span class="text-xs font-medium">{themesAvailable.find(t => t.id === $currentTheme)?.name || 'Theme'}</span>
    </button>
  ...
  <!-- Dropdown menu -->
  {#if isOpen}
    <div class="absolute right-0 top-full mt-1 w-52 rounded-lg shadow-xl z-50 bg-[var(--color-bg-secondary)] border border-[var(--color-border)]">
      <!-- Theme Section -->
      <div class="p-2">
        <div class="text-[9px] font-medium text-[var(--color-text-muted)] mb-1.5 uppercase tracking-wide">Theme</div>
        <div class="flex flex-col gap-0.5">
          {#each themesAvailable as theme}
            <button
              onclick={() => selectTheme(theme.id)}
              class="flex items-center gap-2 px-2 py-1.5 rounded transition-colors {$currentTheme === theme.id
            ...
            >
              ...
              >
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="{theme.id === 'light' ? '#1f2937' : '#f0f0f5'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  {@html themeIcons[theme.id]}
                </svg>
              </div>
              <span class="text-xs text-[var(--color-text)]">{theme.name}</span>
              {#if $currentTheme === theme.id}
                <svg class="w-3 h-3 ml-auto text-[var(--color-accent)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
